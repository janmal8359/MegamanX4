...
void hellFire::fire(float x, float y)
{
	...
	ZeroMemory(&bullet, sizeof(tagBullet));
	bullet.bulletImage = new image;
	//bullet.bulletImage->init("resources/Enemy/Boss/auraBlade.bmp", 0, 0, 192, 232, 3, 2, true, RGB(255, 0, 255));
	if (boss->getDirection() == 0)
	{
		bullet.bulletImage->init("resources/Enemy/Boss/auraBladeLeft.bmp", 0, 0, 192, 116, 3, 1, true, RGB(255, 0, 255));
		bullet.speed = 10.f;
	}
	else if (boss->getDirection() == 1)
	{
		bullet.bulletImage->init("resources/Enemy/Boss/auraBladeRight.bmp", 0, 0, 192, 116, 3, 1, true, RGB(255, 0, 255));
		bullet.speed = -10.0f;
	}
	bullet.x = bullet.fireX = x;
	bullet.y = bullet.fireY = y;
